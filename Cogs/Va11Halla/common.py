...
from Utils import progress_bar, CommandArgumentError
from .reader import Va11HallaJSON, CharacterNotFound, ScriptLineDoesNotExists
...
class Va11Halla:
    ...
    def va11_list(self, list_type, page=1):
        try:
    ...
    async def va11halla(self, ctx, character, script, line):
        if character and line:
            return "Пожалуйста, не ищите линию и персонажа одновременно", CommandArgumentError("va11halla")
        elif line and not script:
            return "Пожалуйста, введите скрипт для начала", CommandArgumentError("va11halla")
        if script and line:
            try:
                dial = self.data.get_script_line(line_num=line, script_name=script)
            except ScriptLineDoesNotExists as e:
                return "Выберите линию входящую в диапозон от 2 до " + str(e.script.lines)
